...
void ShootBullet_Missile(ShootInfo* sData, int level, BOOL bSuper)
{
	...
	if (sData->keyTrg & GetKeybind(CSM_KEYBIND_DESC::CSM_KEYBIND_DESC_SHOOT))
	{
		...
		PlaySoundObject(32, SOUND_MODE_STOP_THEN_PLAY);
	}
}
...
void ShootBullet_DebugWeapon(ShootInfo* sData, int level)
{
	if ((sData->keyTrg & GetKeybind(CSM_KEYBIND_DESC::CSM_KEYBIND_DESC_SHOOT)) && ((gStageNo == 138 || gStageNo == 139 || gStageNo == 140) || (sData->ourC->xm == 0 && sData->ourC->ym == 0)))
	{
		CARET* pSpawnedCaret = NULL;

		// Spawn the smiley caret!
		SetCaret(sData->ourC->x, sData->ourC->y, CaretID_Smiley, sData->ourC->up ? Directions::DIRECT_UP : (sData->ourC->down ? Directions::DIRECT_DOWN : sData->ourC->direct), &pSpawnedCaret);

		// Init the spawned caret further
		if (pSpawnedCaret != NULL)
			pSpawnedCaret->xm = level - 1;

		// Play the happy ping!
		PlaySoundObject2D(sData->ourC->x, sData->ourC->y, 0x2000, true, 157, SOUND_MODE_STOP_THEN_PLAY, 255);
	}

#ifdef _DEBUG

	// But also shoot the gun
	if (sData->keyTrg & GetKeybind(CSM_KEYBIND_DESC::CSM_KEYBIND_DESC_SHOOT))
	{
		if (sData->ourC->up)
		{
...
void ActCaret_Smiley(CARET* crt)
{
	// crt->xm = Weapon Level
	// crt->ym = Target Y

	// Get our rect!
	RECT rcSmiley    = { 0, 240, 20, 256 };
	rcSmiley.left   += 20 * crt->xm;
	rcSmiley.right  += 20 * crt->xm;
	rcSmiley.top    += 16 * crt->direct;
	rcSmiley.bottom += 16 * crt->direct;
	crt->rect        = rcSmiley;
	...
	// Stop if we're done!
	if (gStageNo == 138 || gStageNo == 139 || gStageNo == 140)
	{
		// Last longer
		if (crt->ani_wait >= 40)
			crt->cond = 0;
	...
	{
		// Don't last as long, so we don't block bosses
		if (crt->ani_wait >= 20)
			crt->cond = 0;
	}
}
