...
void ShootBullet_Bubblin2(ShootInfo* sData, int level)
{
	...
	if (sData->key & gKeyShot)
	{
		...
		else
		{
			...
			PlaySoundObject(48, SOUND_MODE_STOP_THEN_PLAY);
		}
	}
	else if (++wait > 1)
	{
...
void ActBullet_Bubblin3(BULLET* bul, int level)
{
	...
	if (++bul->act_wait > 100 || !((pClient && !pClient->mCache.is_our_player ? pClient->mCache.gameKey : gKey) & GetKeybind(CSM_KEYBIND_DESC::CSM_KEYBIND_DESC_SHOOT)))
	{
		bul->cond = 0;
		SetCaret(bul->x, bul->y, 2, 0);
