...
void ActBullet_Nemesis(BULLET *bul, int level)
{
	...
	else
	{
		...
		{
			// September 5th, 2024:  Do not spawn smoke if we're in multiplayer (because it's annoying)
			if (level == 1 && (bul->count1 % 4 == 1) && !CSM_CaveNet_IsHosting() && !CSM_CaveNet_ConnectedAsClient())
			{
				switch (bul->direct)
				{
					case 0:
